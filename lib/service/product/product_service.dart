...
class ProductService {
  ...
  static Future<dynamic> FollowedProductByUser(FollowedProductByUserId) async{
    ...
    final response = await ClientWithInterception.client.get(
        Uri.parse(APIEndpoint.endPointUri(baseUrl: BASE_PATH, customUrl: GET_ALL_PATH)
            .toString() +
  ...
  static Future<dynamic> getProductbyDistance({Longitude,Latitude,DistanceEnum}) async{
    print(RequestHeader.getHeader());
    final response = await ClientWithInterception.client.get(
        Uri.parse(APIEndpoint.endPointUri(baseUrl: BASE_PATH, customUrl: GET_ALL_PATH)
            .toString() +
  ...
  static Future<dynamic> getProductLikebyUser(userId) async {
    ...
    print(RequestHeader.getHeader());
    final response = await ClientWithInterception.client.get(
        Uri.parse(APIEndpoint.endPointUri(baseUrl: BASE_PATH, customUrl: "GetProductLikeListByUser")
            .toString() +
            "?userId=$userId"),
        headers: RequestHeader.getHeader()
    );
  ...
  static Future<dynamic> uploadProduct(File imageFile) async {
    if (await imageFile.exists()) {
     ...
     // print('mimeTypedData: $mimeTypedData');
      ...
      final uri = APIEndpoint.endPointUri(baseUrl: FILE_PATH, customUrl: 'Upload');
  ...
  static Future<dynamic> uploadProduct2(List<String> profileImages) async {
    ...
    if (await profileImages.isNotEmpty) {
      ...
      for(int i = 0 ; i<profileImages.length;i++){
        ...
        mimeTypedData = lookupMimeType(profileImages[i], headerBytes: [0xFF, 0xD8]).split("/");
        print(mimeTypedData);
      }
      final uri = APIEndpoint.endPointUri(baseUrl: FILE_PATH, customUrl: 'UploadFiles');
      final imageUploadRequest = http.MultipartRequest("POST", uri);
      // final file = await http.MultipartFile.fromPath("file", imageFile.path,
      //     contentType: MediaType(mimeTypedData[0], mimeTypedData[1]));
      // String images = "";
     ...
     // var file;
      for (String item in profileImages) {
        mimeTypedData = await lookupMimeType(item, headerBytes: [0xFF, 0xD8]).split("/");
        imageUploadRequest.files.add(await http.MultipartFile.fromPath('files', item, contentType:MediaType(mimeTypedData[0], mimeTypedData[1])));
      }
