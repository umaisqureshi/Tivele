...
import 'package:tivele/Provider/productCategory.dart';
...
class _UploadItemState extends State<UploadItem> {
  ...
  Widget build(BuildContext context) {
    double width = MediaQuery
        ...
        .size
        .width;
    final categorys = context
        .watch<ProductsCategoryProvider>()
        .category;
    return ModalProgressHUD(
      inAsyncCall: isUploading,
      progressIndicator: CircularProgressIndicator(
          backgroundColor: Colors.black.withOpacity(0.8),
          valueColor: new AlwaysStoppedAnimation<Color>(Colors.white.withOpacity(1))),
      child: SafeArea(
        child: Scaffold(
                ...
                children: [
                  Container(
                      ...
                      child: Stack(children: [
                        ClipRRect(
                            ...
                            child: _newProfileImage ? picsListView()
                  ...
                  Container(
                    ...
                    child: InputDecorator(
                      decoration: InputDecoration(
                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(5.0), borderSide:  BorderSide(color: Colors.white, width: 4)),
                        contentPadding: EdgeInsets.all(10),
                      ),
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          hint: new Text("Select Category", style: GoogleFonts.poppins(color: Colors.white70),),
                          underline: Container(
                            color: Colors.red,
                          ),
                  ...
                  Center(
                    child: ConfirmationSlider(
                      onConfirmation: () {
                        CreateProduct();
                        if(isbusy==true)Center(child: CircularProgressIndicator(
                            backgroundColor: Colors.white,
                            valueColor: new AlwaysStoppedAnimation<Color>(
                                Colors.black)
                        ));
  ...
  Widget priceTextField(int Index, String HintText, TextEditingController Controller){
    return TextField(
      style: Global.style(
        size: 16,
      ),
        ...
        setState(() {
          if(Index==0){
            if(newPriceController.text.isNotEmpty){
              try{
                gstController.text = ((13/100)*double.parse(newPriceController.text)).toStringAsFixed(2);
                newPriceError = "";
              }catch(e){
      ...
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.white, width: 4)),
  ...
  Widget picsListView(){
    return PageView.builder(
      ...
      itemBuilder: (context, index) => pic_listget(selectedMedias[index], index),
    );
  }
  Widget pic_listget(File selectedMedia, int index){
        return checkMedia[index]=="image"?Container(
  ...
  void _getImage() async {
    if(selectedMedias.length!=3){
      ...
      showErrorToast("You have selected all the 3 medias.\nTry clear medias to insert again.");
    }
  ...
  Future<bool> validate() async {
    ...
    if (DescripationController.text.contains("@") || DescripationController.text.contains("#") || DescripationController.text.contains("%") || DescripationController.text.contains("\$") || DescripationController.text.contains("&")) {
      showErrorToast("Remove Any specail chracter from Description\n(@ # % \$ &) etc. ");
      return false;
    }
  ...
  CreateProduct() async {
    ...
    try {
      ...
      for(int i = 0 ; i<fileData.length; i++){
        description = description + fileData[i]["url"] + "@#@#@";
      }
