...
import 'package:tivele/controllers/customer/getTiveleDataController.dart';
...
class _TPageState extends State<TPage> {
  ...
  final GetTiveleDataController getTiveleDataController = Get.put(GetTiveleDataController());
  ...
  void getInitialDataWithoutLocation() async {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (getTiveleDataController.tiveleDataINfo.value.status == 200) {
        setState(() {
          productDataList.addAll(getTiveleDataController.tiveleDataINfo.value.data);
          isBusy = false;
          isListNull = false;
          page = 1;
        });
                            ...
                            GestureDetector(
                              onTap: () {
                                ...
                                if(globals.checkLogin){
                                  ...
                                  checkLocationToBookOrder(productD, productDataList[index].likes, productDataList[index].rating,productDataList[index].isLiked,productDataList[index] );
                                }
                                },
                              child: Container(
                                child: ClipRRect(
                                    ...
                                    onPressed: () {
                                      if(globals.checkLogin){
                                        ...
                                        setState(() {
                                          ...
                                          _pressed(productDataList[index].id);
                                        });
                                      }
  ...
  checkLocationToBookOrder(List<ProductM> productD, int likes, String rating, bool isLiked, ProductDatum productDataList) async {
    serviceEnabled = await location.serviceEnabled();
    if (!serviceEnabled) {
      print("asli");
      serviceEnabled = await location.requestService();
