...
import 'package:tivele/controllers/customer/customerLikeController.dart';
import 'package:tivele/controllers/customer/orderPlacedController.dart';
...
import 'package:tivele/models/productModel.dart';
...
class _BusinessDetailPageState extends State<CustomerDetailPage> {
  ...
  String burger = 'https://biancazapatka.com/wp-content/uploads/2020/05/veganer-bohnen-burger-500x500.jpg';
...
*/
    ...
    List<geo.Placemark> placemarks = await geo.placemarkFromCoordinates(double.parse(productD[0].productLatitude), double.parse(productD[0].productLongitude));
                              ...
                              child: Row(
                                ...
                                children: [
                                  RatingBar.builder(
                                    ...
                                    initialRating: rating==null?5:double.parse(rating),
                                    minRating: 1,
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        productDetailsDatum.productAttributes!=[]?addExtraList():Container(),
                        Divider(
                          color: Colors.white,
                        ),
                        SizedBox(height: 12),
                                ...
                                onPressed: () {
                                  setState(() {
                                    ...
                                    if (orderIndex == 0) {
                                      ...
                                      orderTotal = (OrderTotalAmount + tipValue)
                                          .toStringAsFixed(1);
                                      GrandTotalAmount = order.shippingCost +
                                          ProductAmount +
                                          TiveleFee +
                        ...
                        SizedBox(height: 24),
                        !checkTip && Platform.isAndroid?Align(
                          alignment: Alignment.center,
                          child: pay.GooglePayButton(
                            ...
                            paymentItems: [
                              pay.PaymentItem(
                                label: 'Total',
                                amount: double.parse(GrandtotalAmountText).ceilToDouble().toInt().toString(),
                                status: pay.PaymentItemStatus.final_price,
                              )
                            ],
                        ...
                        !checkTip && Platform.isIOS && Stripe.instance.isApplePaySupported.value?
                        pay.ApplePayButton(
  ...
  Widget addExtraList(){
    return ListView.builder(
      ...
      itemBuilder: (context, index) => extraListGet(index, productDetailsDatum.productAttributes[index]),
    );
  }
              ...
              Row(
                ...
                children: [
                  ...
                  InkWell(
                    onTap: (){
                      setState(() {
                        if(selectedProductAttributes.any((attribute) => attribute.productAttributeId==productAttribute.attributeValues[index].productAttributeId)){
                          if(selectedProductAttributes.any((attribute) => attribute.id==productAttribute.attributeValues[index].id)){
                            selectedProductAttributes.removeWhere((attribute) =>  attribute.id==productAttribute.attributeValues[index].id);
                          }else{
                            int indexValue = selectedProductAttributes.indexWhere((attribute) => attribute.productAttributeId == productAttribute.attributeValues[index].productAttributeId);
                            selectedProductAttributes.removeAt(indexValue);
                            selectedProductAttributes.add(productAttribute.attributeValues[index]);
                          }
                        }else{
                          selectedProductAttributes.add(productAttribute.attributeValues[index]);
                        }
                    ...
                    child: Container(
                      ...
                      child: selectedProductAttributes.any((attribute) => attribute.id==productAttribute.attributeValues[index].id)?Container(
                        decoration: BoxDecoration(
                          color: Colors.white,
                          shape: BoxShape.circle,
                        ),
...
*/
  ...
  void setError(dynamic error) {
    _scaffoldKey.currentState.showSnackBar(
        SnackBar(content: Text((error as PlatformException).message)));
    setState(() {
  ...
  Future<void> onGooglePayResult(paymentResult) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Your payment succesfully completed')),
      );
    } catch (e) {
      print("error");
      print(e);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Your payment not submitted!\nPlease try sgain.')),
      );
    }
  }

  Future<Map<String, dynamic>> fetchPaymentIntentClientSecret() async {
    final url = Uri.parse("https://us-central1-tivele-25c51.cloudfunctions.net/stripePayment");
//    final response = await http.post(url);
  ...
  Future<void> debugChangedStripePublishableKey() async {
    if (kDebugMode) {
      ...
      final isValidKey = !profile.contains('pk_live_3ayggMnNp5g9v2tJdRbPA5sO00UlZ9bk9L');
      assert(
      isValidKey,
