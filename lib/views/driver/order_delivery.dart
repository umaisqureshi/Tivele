...
import 'package:tivele/controllers/driver/orderDeliveredByDriverController.dart';
...
class _OrderDeliveryState extends State<OrderDelivery> {
                                  ...
                                  ? Stack(
                                    children: [
                                        GoogleMap(
                                          ...
                                          markers: {
                                            Marker(
                                              position: LatLng(double.parse(widget.order.userLatitude), double.parse(widget.order.userLongitude)),
                                          ...
                                          initialCameraPosition: CameraPosition(
                                            target: LatLng(double.parse(widget.order.userLatitude), double.parse(widget.order.userLongitude)),
                                            zoom: 15.0,
                                          ),
                                          ...
                                          child: RaisedButton(
                                              ...
                                              onPressed: ()async{
                                                ...
                                                await availableMaps.first.showMarker(
                                                  coords: Coords(double.parse(widget.order.userLatitude), double.parse(widget.order.userLongitude)),
                                                  title: "Dropoff Order Place",
                                                );
  ...
  uploadPic(File file) async {
    ...
    String medias = "";
    await uploadPicController.fetchPic(file, file.path.split("/").last.toString(), 4, "Pickp Selfie");
    if (uploadPicController.picINfo.value.status == 200) {
      print("delivery pic path");
      print(uploadPicController.picINfo.value.data);
      await acceptOrderDeliveredController.fetchorderdeliveredbydriver(uploadPicController.picINfo.value.data, widget.order.id);
